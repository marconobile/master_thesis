...
#        mols = [mol.UpdatePropertyCache() if mol is not None else None for mol in mols]
#        scores = [MolecularMetrics._avoid_sanitization_error(lambda: Crippen.MolLogP(mol)) if mol is not None else None
#                  for mol in mols]

        # mols_  = [Chem.SanitizeMol(mol) if mol is not None else None
        #           for mol in mols]

        # scores = [Crippen.MolLogP(mol) if mol is not None else None
        #           for mol in mols]
        
        #mols_ = [mol.UpdatePropertyCache() if mol is not None else None for mol in mols]
#        mols_ = [MolecularMetrics._avoid_sanitization_error(lambda: FastFindRings(mol)) if mol is not None else None for mol in mols]
