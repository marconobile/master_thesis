...
# from rnns_check_params import check_params, freeze_net
from reward_structure import get_mol, set_y_single_obs
from args import Args


def train_rnn_epoch(epoch, rnn, output, data_loader_, optimizer_rnn, optimizer_output,
                    scheduler_output, scheduler_rnn, node_weights, edge_weights, device, absence_net, absence_net_opt,
                    scheduler_abs):
    ...
    for batch_idx, data in enumerate(data_loader_):
        ...
        y_reshape = y_reshape.view(y_reshape.size(0), max_prev_node, 5)  # .view() to ensure contiguity in memory
        output_x = torch.cat((torch.ones(y_reshape.size(0), 1, 5), y_reshape[:, 0:-1, :]), dim=1)
        output_y = y_reshape
        output_y_len = []
        ...
        # TARGETS
        if absence_net:
            output_sigmoid, output_y = torch.split(output_y, [1, 4], dim=2) # qui ho gia il target nuovo

        # INPUTS
        # if absence_net:
        #     abs_f, edge_f = torch.split(output_x, [1, 4], dim=2)  # qui ho gia il target nuovo
        ...
        if absence_net:
            ...
            edge_loss = F.nll_loss(y_pred, indices_edges, reduction='mean',
                                   weight=edge_weights.to(torch.float32).to(device))
...
#            if scheduler_abs:
#                scheduler_abs.step()
#        if scheduler_rnn and scheduler_output:
...
def test_rnn_single_epoch(epoch, rnn, output, data_loader_, node_weights, edge_weights, device, absence_net):
