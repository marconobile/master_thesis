...
from chem import numpy_to_rdkit, plot_rdkit_svg_grid, valid_score, numpy_to_smiles, novel_score, unique_score
...
def clean_dataset(dataset):
    '''
    cleans dataset from Gs with isolated nodes
    returns a list of UnxG
    '''
    with_iso_nodes = []
    len_ = len(dataset)
    for num, g in enumerate(dataset):
        nxG = to_networkx(g)
        UnxG = to_undirected(nxG)
        if len(list(nx.isolates(UnxG))) != 0:
            with_iso_nodes.append(num)

    with_iso_nodes = set(with_iso_nodes)
    dataset_ = []
    for idx in range(len_):
        if idx not in with_iso_nodes:
            dataset_.append(dataset[idx])

    return dataset_


def encode_adj(adj, original, max_prev_node):
    '''
    ...
    '''
    n = original.shape[0] - 1  # V - 1 of the current graph
    temp = np.zeros((n, max_prev_node, 5))
    ...
    for index in range(len(original_tril_idx[0])):
        ...
        # [i - 1, j ]  cuz since we drop first row of A in the encoding, we need to 'move up' every row-idx

    adj_output = np.zeros((n, max_prev_node, 5))
...
class Graph_sequence_sampler_pytorch(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x_batch = np.zeros((self.max_num_node, self.max_prev_node, 5)) # 5 = edge dims
        y_batch = np.zeros((self.max_num_node, self.max_prev_node, 5)) # 5 = edge dims
...
def mols_smiles_plots(list_of_pygeom_data, name, epoch):
    mols_for_txt = []
    mols_for_plot = []
    smiles_ = []
    for i, obs in enumerate(list_of_pygeom_data):
        ...
        #     pass
        # try:
            #mols_for_plot.append(numpy_to_rdkit(adj, nf, ef, sanitize=True))  # here sanitization for plot only
            try:
                m = numpy_to_rdkit(adj, nf, ef, sanitize=True)
                Chem.Kekulize(m)
                mols_for_plot.append(m)
            except:
                pass
                #print('a graph has been plotted')
                # plot_graph.plot_graph(adj, nf, ef, name="./figures/_Epoch_" + str(epoch) + "_Graph_Num_" + str(i))


            mols_for_txt.append(numpy_to_rdkit(adj, nf, ef, sanitize=False))  # here NO sanitization for txt only
            smiles_.append(numpy_to_smiles(adj, nf, ef))
        except:      # THIS ARE THE ORIGINAL
            #print('failed to build adj, ef,nf')
            pass
    ...
    plot_rdkit_svg_grid(mols_for_plot, mols_per_row=5, filename=name)

    return mols_for_txt, smiles_


def mols_txt(epoch, mols_, smiles_, qm9_smiles):
    # 1) get from the whole list of generated mols, validity,  novelty  and unique_score:

    val_score_list = valid_score(mols_, from_numpy=False)  # if valid or not: T/F
    novel_score_list = novel_score(mols_, qm9_smiles, from_numpy=False)  # if new or not T/F
    unique_score_list = unique_score(mols_, from_numpy=False)
    # the fraction of unique and valid molecules w.r.t. to the number of valid molecules

    # 2) get smiles of (validy AND novel)
    unique_and_valid_smiles = []
    for i in range(len(val_score_list)):
        if (val_score_list[i] == True) and (novel_score_list[i] == True):
            unique_and_valid_smiles.append(smiles_[i])

    # 3) take the set  of unique_and_valid_smiles, to drop duplicates:
    unique_and_valid_smiles = list(set(unique_and_valid_smiles))

    # 4) cast non-duplicate list of smiles to Chem.mols:
    unique_and_valid_mols = [Chem.MolFromSmiles(smile) for smile in unique_and_valid_smiles]

    # 5) for the non-duplicate set of unique and valid mols, get chemical scores:
    drug_like_val = []
    solub_val = []
    synt_val = []
    for mol in unique_and_valid_mols:
        try:
    ...
    with open('./report/mols_smiles_metrics_epoch_' + str(epoch) + '.txt', 'a') as f:
        ...
        for i in range(len(unique_and_valid_mols)):
            f.write(
                f'Mol #{i}, SMILE: {str(unique_and_valid_smiles[i])}, druglikeness_score: {str(drug_like_val[i])}, '
        ...
        f.write(
            ...
            f'Average solubility {np.mean(np.array(synt_val))}')


def get_qm9_smiles(dataset):  # care for updates on dataset!
    smiles_ = []
