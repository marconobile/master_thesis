...
def new_gen(rnn, output, absence_net, device, args, max_num_node, max_prev_node, test_batch_size=1):
    # initialize hidden state
    rnn.hidden = rnn.init_hidden_rand(test_batch_size).to(device)
    # create node level token
    x_step = torch.ones((test_batch_size, 1, max_prev_node * 5 + args.node_feature_dims), requires_grad=False).to(device)
    ...
    for i in range(max_num_node):
        ...
        # ARG-MAX/Gumbel for nodes
        if args.supervised == True:
            idx_node_arg_max = torch.argmax(node_prediction, dim=-1).item()
            node_prediction_gumbel = torch.zeros_like(node_prediction).to(device)  # torch.Size([1, 1, 4])
            node_prediction_gumbel[:, :, idx_node_arg_max] = 1
        elif args.gan == True or args.wgan == True:
            node_prediction_gumbel = F.gumbel_softmax(node_prediction, dim=2, hard=True).to(device)

        # get discretized node-prediction and append it to list
        node_prediction_gumbel_squeezed = node_prediction_gumbel.squeeze(0).to(device)
        x_list.append(node_prediction_gumbel_squeezed)

        # reset and update input for next iteration
        x_step = torch.zeros((test_batch_size, 1, max_prev_node * 5 + args.node_feature_dims), requires_grad=False).to(
            device)
        x_step[:, :, :args.node_feature_dims] = node_prediction_gumbel.data  # .data cuz we only want to get the contenet of the tensor
        ...
        if args.supervised == True:
            ...
            hidden_null_1 = torch.zeros((rnn.num_layers - 1, h.size(0), h.size(2)), requires_grad=True).to(device)
            hidden_null_2 = torch.zeros((rnn.num_layers - 1, h.size(0), h.size(2)), requires_grad=False).to(device)
            h_to_pass = h.permute(1, 0, 2).to(device)
            output.hidden = torch.cat((h_to_pass, hidden_null_1), dim=0).to(device)
            if absence_net:
                h_to_pass_for_abs_net = h_to_pass.data
                absence_net.hidden = torch.cat((h_to_pass_for_abs_net, hidden_null_2), dim=0).to(device)

        # token Edge lvl
        output_x_step = torch.randn(test_batch_size, 1, 5, requires_grad=False).to(device)
        # token abs lvl
        # abs_x_step = torch.ones(test_batch_size, 1, 1, requires_grad=False).to(device)
        abs_x_step = torch.ones(test_batch_size, 1, 5, requires_grad=False).to(device)
        ...
        for j in range(min(max_prev_node, i + 1)):
            ...
            # ARG-MAX/Gumbel for edges
            if args.supervised == True:
                idx_edge_arg_max = torch.argmax(output_y_pred_step_out, dim=-1).item()
                output_x_step_gumbel = torch.zeros_like(output_y_pred_step_out).to(device)  # torch.Size([1, 1, 4])
                output_x_step_gumbel[:, :, idx_edge_arg_max] = 1
            elif args.gan == True or args.wgan == True:
                output_x_step_gumbel = F.gumbel_softmax(output_y_pred_step_out, dim=2, hard=True).to(device)

            # discretization of abs prediction
            if absence_net:
                if abs_.item() >= 0.5:
                    temp_0 = torch.zeros_like(output_x_step_gumbel, requires_grad=True).to(device)
                    output_x_step_gumbel = torch.mul(output_x_step_gumbel,
                                                     temp_0)  # this kills gradients, thus params not updated!
                    t = torch.Tensor([0.5]).to(device)
                    abs_out = (abs_ >= t).float()
                    output_x_step_gumbel = torch.cat((abs_out, output_x_step_gumbel), dim=-1)
                else:
                    t = torch.Tensor([0.5]).to(device)
                    abs_out = (abs_ > t).float()
                    output_x_step_gumbel = torch.cat((abs_out, output_x_step_gumbel), dim=-1)
                ...
                # reset and update input for next iteration
                abs_x_step = output_x_step_gumbel  # abs_out.data

            if torch.argmax(output_x_step_gumbel, dim=-1) != 0:
                if i + 1 <= max_prev_node:
                    # select [1:]
                    idx_select = torch.tensor([1, 2, 3, 4], dtype=torch.long, requires_grad=False).to(device)
                    edge_to_append = torch.index_select(output_x_step_gumbel, dim=2, index=idx_select).to(device)
            ...
            x_step[:, :, 4 * j + args.node_feature_dims + j: 4 * (j + 1) + args.node_feature_dims + (j + 1)] = output_x_step_gumbel.data
            edge_rnn_step = j
